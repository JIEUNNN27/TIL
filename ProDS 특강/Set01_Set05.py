...
data1.info()
data1.columns #컬럼의 이름
# ['TV', 'Radio', 'Social_Media', 'Influencer', 'Sales']컬럼명 복사해두기
data1.values #이걸 하면 numpy로 변경
...
#data1.isna().sum(axis = 1) #행별로 결측치
data1.isna().sum() #열별로 결측치
...
# 공분산과 상관계수 -> 아이패드 메모장에 필기함!

# 특정 변수만 뽑아내기
var_list = ['TV', 'Radio', 'Social_Media', 'Sales']
#data1[var_list].corr()
#data1[var_list].corr().drop('Sales')
#data1[var_list].corr().drop('Sales')['Sales']
q2_corr = data1[var_list].corr().drop('Sales')['Sales'].abs()
#q2_corr

q2_corr.max() #최대값
q2_corr.idxmax() #최대값이 있는 인덱스명
q2_corr.argmax() #최대값이 있는 위치번호(인덱스)
q2_corr.nlargest(2)  #최대값 상위 2개
q2_corr.nlargest(2).index

# 채널의 상관계수 소수점 5번째 자리에서 
#반올림하여 소숫점 넷째 자리까지 기술
round(q2_corr.max(),4)
...
q3 = data1.dropna()
lm1 = LinearRegression(fit_intercept = True).fit(q3[var_list], q3['Sales'])      #절편 포함 회귀식 => True, 절편 불포함 => False
...
(np.floor(coef1.sort_values(ascending = False)*1000)/1000).values #True: 오름차순, False : 내림차순
...
form2 = 'Sales~' + '+'.join(var_list)   #'+'.join(var_list) 이렇게 하면 문자열
...
q2 = data2.copy()
# np.where(조건)
# np.where(조건[True/False], 참인경우 실행[함수(식), 상수, 시리즈(array)], 거짓인 경우 실행)
...
# 2. 반복적 카이제곱 검정
# - 반복적으로 두 변수간의 카이제곱 검정
# - 반복적으로 사용할 변수 목록
# - for 루프 안에서 카이제곱검정이 진행 (입력값은 빈도표로 들어가도록 설계)

from scipy.stats import chi2_contingency
#이거 사용하려면 입력표에 빈도 표로 들어가도록 설계해야 한다...
...
for i in var_list:
    ...
    chi_out = chi2_contingency(tab) #카이제곰 결과
    pvalue = chi_out[1]
