...
# In our experiments, each paragraph vector is a combination of two vectors: 
# one learned by the standard paragraph vector with distributed memory (PV-DM) 
# and one learned by the paragraph vector with distributed bag of words (PVDBOW). 
# PV-DM alone usually works well for most tasks (with state-of-art performances), 
...
model1 = Doc2Vec(tag_sent, vector_size=32, window=5, min_count=1, dm=1)
model2 = Doc2Vec(tag_sent, vector_size=32, window=5, min_count=1, dm=0)

d_vector = [np.hstack([model1.docvecs[tags[0]], model2.docvecs[tags[0]]]) for _, tags in tag_sent]
